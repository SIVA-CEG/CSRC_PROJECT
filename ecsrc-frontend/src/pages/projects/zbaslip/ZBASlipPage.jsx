...
const STAFF_LIST = [
  {
    ...
    hra: "Rs.7,440/-",
    projectTitle:
      "M.H.No.10.1.121—Development of Ti(C,N) based cermets modified by Si3N4, B4C and Cr3C2 for metal cutting application",
...
const SAMPLE_CLAIMS = [
  {
    ...
    claimAmount: 85560.0,
    rows: [
      { from: "14-05-2023", upto: "31-05-2023", cl: 0.0, lop: 0.0 },
      { from: "01-06-2023", upto: "30-06-2023", cl: 0.0, lop: 0.0 },
      { from: "01-07-2023", upto: "20-07-2023", cl: 0.0, lop: 0.0 },
    ],
  },
  {
    id: 2,
    staffId: 1,
    salaryFrom: "01-04-2023",
    salaryTo: "30-04-2023",
    clDays: 1.0,
    lopDays: 0.0,
    claimDays: 30,
    claimAmount: 38440.0,
    rows: [{ from: "01-04-2023", upto: "30-04-2023", cl: 1.0, lop: 0.0 }],
  },
];
...
function calcNetSalary(from, upto, salaryPerMonth, cl, lop) {
  ...
  return Math.round(dailyRate * days - lop * dailyRate);
}
function fmtAmt(n) {
  ...
  return [
    {
      ...
      percent: 5,
      amount: Math.round(project.sanctionedAmount * 0.05),
    },
    {
      key: "csrcRevenue",
      label: "CSRC Revenue, Chennai",
      percent: 4,
      amount: Math.round(project.sanctionedAmount * 0.04),
    },
    {
      key: "dean",
    ...
    },
    {
      ...
      percent: 2,
      amount: Math.round(project.sanctionedAmount * 0.02),
    },
  ];
}
...
function generateManpowerCoverPage(claim, staff, project) {
  const td = todayDMY();
  const totalClaim = claim.rows.reduce(
    (s, r) =>
      s + calcNetSalary(r.from, r.upto, staff.salaryPerMonth, r.cl, r.lop),
    0,
  );
...
function generateTravelCoverPage(formData, project) {
  ...
  const claimantRows = `
    ...
    <tr><td>Travel Date</td><td>${formatDateDMY(formData.travelDate) || "—"}${formData.duration ? ` (${formData.duration})` : ""}</td></tr>
  ...
  return coverPageShell({
    ...
    subLine: `JOURNEY FROM ${(formData.fromPlace || "—").toUpperCase()} TO ${(formData.toPlace || "—").toUpperCase()} ON ${formatDateDMY(formData.travelDate) || "—"}`,
    dateStr: td,
    projectRows,
    piRows,
...
function combineCoverAndProceedings(coverHTML, proceedingsHTML) {
  ...
  const cleanProcBody = procBody.replace(
    /<button[^>]*print-btn[^>]*>.*?<\/button>/gi,
    "",
  );
  ...
  const cleanCoverBody = coverBody.replace(
    /<button[^>]*print-btn[^>]*>.*?<\/button>/gi,
    "",
  );
...
function combineMultipleCoverAndProceedings(pairs) {
  ...
  pairs.forEach(({ coverHTML, proceedingsHTML }) => {
    const coverBodyMatch = coverHTML.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
    const coverBody = (coverBodyMatch ? coverBodyMatch[1] : coverHTML).replace(
      /<button[^>]*print-btn[^>]*>.*?<\/button>/gi,
      "",
    );
    const coverStyleMatch = coverHTML.match(
      /<style[^>]*>([\s\S]*?)<\/style>/gi,
    );
    if (coverStyleMatch) styles += coverStyleMatch.join("\n") + "\n";

    const procBodyMatch = proceedingsHTML.match(
      /<body[^>]*>([\s\S]*?)<\/body>/i,
    );
    const procBody = (
      procBodyMatch ? procBodyMatch[1] : proceedingsHTML
    ).replace(/<button[^>]*print-btn[^>]*>.*?<\/button>/gi, "");
    const procStyleMatch = proceedingsHTML.match(
      /<style[^>]*>([\s\S]*?)<\/style>/gi,
    );
...
function generateConsumablesPDF(formData, project) {
  const td = new Date()
    ...
    .replace(/\//g, ".");
  const balance =
    project.budgetAllotment -
    (project.amountIncurredSoFar + Number(formData.amount));
  const amountIncludingThis =
    project.amountIncurredSoFar + Number(formData.amount);
...
function generateTravelPDF(formData, project) {
  ...
  const claimedSoFar = Number(formData.claimedSoFar || 0);
  const amountIncludingThis = claimedSoFar + Number(formData.amount || 0);
  const balance = budget - amountIncludingThis;
...
<div class="page">
  ...
  <div class="stars">*****</div>
  <p class="para indent">Sanction is hereby accorded for an amount of <strong>Rs.${Number(formData.amount).toLocaleString("en-IN")}/- (Rupees ${toIndianWords(Number(formData.amount))})</strong> towards travelling allowance by ${formData.modeOfTravel || "own car"}${formData.vehicleNo ? ` (No.${formData.vehicleNo})` : ""} and DA charges on 
  <strong>${formatDateDMY(formData.travelDate)}${formData.duration ? ` (${formData.duration})` : ""}</strong> from <strong>${formData.fromPlace || "Chennai"}</strong> to <strong>${formData.toPlace}</strong> and return back to ${formData.fromPlace || "Chennai"} in connection with ${formData.purpose || "field work"} under ${project.scheme} project no.${project.projectNo} entitled "<strong>${project.title}</strong>".</p>
  ...
  <div class="committed-section"><div class="commit-header"><span></span><span></span><span class="committed-title">Committed</span></div>
  <div class="commit-row"><span style="font-size:22px;">•</span><span>Budget Allotment in BE/RE</span><span class="commit-amount">Rs. ${Number(project.budgetAllotment).toLocaleString("en-IN")}/-</span></div>
  <div class="commit-row"><span style="font-size:22px;">•</span><span>Amount incurred so far<br>(Including this proceeding)</span><span class="commit-amount">Rs. ${Number(amountIncludingThis).toLocaleString("en-IN")}/-</span></div>
  <div class="commit-row"><span></span><span>Balance amount available</span><span class="commit-amount">Rs. ${Number(balance).toLocaleString("en-IN")}/-</span></div></div>
...
<div class="page">
  ...
  <div class="committed-section"><div class="commit-header"><span></span><span></span><span class="committed-title">Committed</span></div>
  <div class="commit-row"><span style="font-size:22px;">•</span><span>Budget Allotment in BE/RE</span><span class="commit-amount">Rs. ${Number(budget).toLocaleString("en-IN")}/-</span></div>
  <div class="commit-row"><span style="font-size:22px;">•</span><span>Amount incurred so far<br>(Including this proceeding)</span><span class="commit-amount">Rs. ${Number(amountIncludingThis).toLocaleString("en-IN")}/-</span></div><div class="commit-row"><span></span><span>Balance amount available</span><span class="commit-amount">Rs. ${Number(balance).toLocaleString("en-IN")}/-</span></div></div>
...
function generateNonRecurringPDF(formData, project) {
  const td = new Date()
    ...
    .replace(/\//g, ".");
  const equipmentBudget = Number(formData.equipmentBudget || 0);
  const amountIncludingThis = Number(formData.amount || 0);
  const balance = equipmentBudget - amountIncludingThis;
...
<div class="page">
  ...
  <div class="bullet-section">
    <div class="bullet-row"><span class="bullet-dot">•</span><span class="bullet-label">Budget Allotment in BE/RE <span class="bullet-dots">Rs. ${fmtAmt(equipmentBudget)}/-</span></span></div>
    <div class="bullet-row"><span class="bullet-dot">•</span><span class="bullet-label">Amount incurred so far<br>&nbsp;&nbsp;&nbsp;(Including this proceeding) <span class="bullet-dots">Rs. ${fmtAmt(amountIncludingThis)}/-</span></span></div>
    <div class="bullet-row"><span class="bullet-dot" style="visibility:hidden;">•</span><span class="bullet-label">Balance amount available <span class="bullet-dots">Rs. ${fmtAmt(balance)}/-</span></span></div>
  </div>
...
function buildCombinedReport(headKey, formData, project, claimData, staff) {
  if (headKey === "overhead") {
    const allItems =
      formData.overheadItems && formData.overheadItems.length
        ? formData.overheadItems
        : claimData.items || [];
    const selected =
      formData.selectedOverheads && formData.selectedOverheads.length
      ...
      <div className="cons-section-body">
        <div className="cons-grid-2" style={{ gap: 14 }}>
          <div className="cons-field">
            <label>
              Project Title <span className="src-badge src-fetch">Fetched</span>
            </label>
          ...
          <div className="cons-field">
            <label>
              Project No. <span className="src-badge src-fetch">Fetched</span>
            </label>
          ...
          <div className="cons-field">
            <label>
              PI Name & Designation{" "}
              <span className="src-badge src-fetch">Fetched</span>
            </label>
            <div className="cons-static">
              <strong>{project.pi}</strong>, {project.piDesignation}
            </div>
          </div>
          <div className="cons-field">
            <label>
              Scheme <span className="src-badge src-fetch">Fetched</span>
            </label>
            <div className="cons-static">{project.scheme}</div>
          </div>
          <div className="cons-field">
            <label>
              Department <span className="src-badge src-fetch">Fetched</span>
            </label>
            <div className="cons-static">
              {project.departmentFull}, {project.campus}
            </div>
          </div>
          <div className="cons-field">
            <label>
              CSRC Proceedings Ref{" "}
              <span className="src-badge src-fetch">Fetched</span>
            </label>
            <div className="cons-static">
              {project.csrcProcNo} dated {project.csrcProcDate}
            </div>
          </div>
          <div className="cons-field">
            <label>
            ...
            <div className="cons-static">
              Rs. {Number(project.sanctionedAmount).toLocaleString("en-IN")}/-
            </div>
          </div>
          <div className="cons-field">
            <label>
              Date <span className="src-badge src-auto">Auto</span>
            </label>
...
function BalanceBar({
  ...
  const b =
    budget !== undefined
      ? Number(budget)
      : Number(project?.budgetAllotment || 0);
  const c =
    claimedSoFar !== undefined
      ? Number(claimedSoFar)
      : Number(project?.amountIncurredSoFar || 0);
  const incl = c + Number(amount || 0);
  const balance = b - incl;
  return (
    <div className="balance-bar">
      <div className="bal-item">
        <div className="bal-label">
          {budgetLabel || "Budget Allotment (BE/RE)"}
        </div>
        <div className="bal-val blue">₹{b.toLocaleString("en-IN")}</div>
      </div>
      <div className="bal-item">
        <div className="bal-label">
          {incurredLabel || "Incurred (incl. this)"}
        </div>
        <div className="bal-val yellow">₹{incl.toLocaleString("en-IN")}</div>
      </div>
      <div className="bal-item">
        ...
        <div className="cons-grid-2" style={{ gap: 16 }}>
          <div className="cons-field">
            <label>
              ...
              <span className="src-badge src-enter">Enter</span>
            </label>
            <input
              type="text"
              className="slip-input"
              placeholder="e.g. 08 / RAC /CMRG/ 2025-26"
              value={data.proceedingNo || ""}
              onChange={(e) => set("proceedingNo", e.target.value)}
            />
          </div>
          <div className="cons-field">
            <label>
            ...
            <input
              type="text"
              className="slip-input"
              placeholder="e.g. R&AC, Thermal..."
              value={data.divisionLabel || ""}
              onChange={(e) => set("divisionLabel", e.target.value)}
            />
          </div>
          <div className="cons-field">
            <label>
            ...
            <input
              ...
              value={data.sanctioningAuthority || ""}
              onChange={(e) => set("sanctioningAuthority", e.target.value)}
            />
          </div>
          <div className="cons-field">
            <label>
              M.H. No. <span className="src-badge src-enter">Enter</span>
            </label>
            <input
              type="text"
              className="slip-input"
              placeholder="e.g. 16.1.17"
              value={data.mhNo || ""}
              onChange={(e) => set("mhNo", e.target.value)}
            />
          </div>
          <div className="cons-field">
            <label>
              Financial Year <span className="src-badge src-enter">Enter</span>
            </label>
            <input
              type="text"
              className="slip-input"
    ...
    <div className="cons-section">
      ...
      <div className="cons-section-body">
        <div className="cons-grid-2" style={{ gap: 16 }}>
          <div className="cons-field">
            <label>
              Vide Page No. <span className="src-badge src-enter">Enter</span>
            </label>
            <input
              type="text"
              className="slip-input"
              placeholder="Page number"
              value={data.sanctionPageNo || ""}
              onChange={(e) => set("sanctionPageNo", e.target.value)}
            />
          </div>
          <div className="cons-field">
            <label>
              Sl. No. <span className="src-badge src-enter">Enter</span>
            </label>
            <input
              type="text"
              className="slip-input"
              placeholder="Serial number"
              value={data.sanctionSlNo || ""}
              onChange={(e) => set("sanctionSlNo", e.target.value)}
            />
          </div>
        </div>
      </div>
...
function ManpowerPage({ onBack, selectedProject }) {
  ...
  const openCombinedPreview = (claim, staff) => {
    const project = selectedProject || DUMMY_PROJECT;
    const combinedHTML = buildCombinedReport(
      "manpower",
      {},
      project,
      claim,
    ...
    return (
      <div className="ssc-page">
        <div className="ssc-header">
          ...
          <div className="ssc-breadcrumb">
            <span className="ssc-bc-link">ZBA</span> › Recurring › Manpower
          </div>
        </div>
                ...
                {claims.map((c, i) => {
                  ...
                  return (
                    <tr key={c.id}>
                      ...
                      <td>{c.salaryFrom}</td>
                      <td>{c.salaryTo}</td>
                      <td>{c.clDays.toFixed(1)}</td>
                      <td>{c.lopDays.toFixed(1)}</td>
                      <td>{c.claimDays}</td>
                      <td className="ssc-amount">{fmtAmt(c.claimAmount)}</td>
                      <td>
                        <div className="ssc-actions">
                          <button
                            className="ssc-action-btn ssc-edit"
                            onClick={() => {
                              setSelectedClaim(c);
                              setView("edit");
                            }}
                          ...
                          <button
                            className="ssc-action-btn ssc-view"
                            onClick={() => {
                              setSelectedClaim(c);
                              setView("report");
                            }}
                          ...
                          <button
                            className="ssc-action-btn ssc-redo"
                            onClick={() => {
                              if (window.confirm("Reset this claim?"))
        ...
        <div className="ssc-list-footer">
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <button className="ssc-btn ssc-btn-back" onClick={onBack}>
              ← Back
            </button>
          ...
          <button
            className="ssc-btn ssc-btn-new"
            onClick={() => {
              setNewStaff(null);
              setNewRows([{ from: "", upto: "", cl: "", lop: "" }]);
              setView("new");
            }}
          >
            + New Salary Claim
      ...
      <div className="ssc-page">
        ...
        <div className="ssc-report-actions">
          <button
            ...
            onClick={() => openCombinedPreview(selectedClaim, staff)}
          >
            👁 Preview Full Report (Cover + Pages)
          </button>
          <button
            className="ssc-btn ssc-btn-outline"
            onClick={() => downloadCombinedPDF(selectedClaim, staff)}
          >
            ⬇ Download PDF
          </button>
          <button
            className="ssc-btn ssc-btn-back"
        ...
        <div className="ssc-report-preview">
          <iframe srcDoc={combinedHTML} title="Report" className="ssc-iframe" />
        </div>
      </div>
    );
  }
  ...
  if (view === "edit" && selectedClaim) {
    ...
    const handleUpdate = () => {
      const totalDays = editRows.reduce(
        (s, r) => s + daysBetween(r.from, r.upto),
        0,
      );
      const totalCL = editRows.reduce((s, r) => s + (parseFloat(r.cl) || 0), 0);
      const totalLOP = editRows.reduce(
        (s, r) => s + (parseFloat(r.lop) || 0),
        0,
      );
      const totalAmt = editRows.reduce(
        (s, r) =>
          s +
          calcNetSalary(
            r.from,
    ...
    return (
      <div className="ssc-page">
        <div className="ssc-header">
          <h2>Edit Salary Claim</h2>
        </div>
        <div className="ssc-form-card">
          <div className="ssc-form-subtitle">
            Editing claim for {staff.salutation} {staff.name}
          </div>
          <div className="ssc-info-block" style={{ marginBottom: 16 }}>
            <div>
              <strong>Appointment:</strong> {staff.appointmentFrom} to{" "}
              {staff.appointmentTo}
            </div>
            <div>
              <strong>Total CL availed:</strong> {selectedClaim.clDays}
            </div>
          </div>
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setSelectedClaim({ ...selectedClaim, rows: r });
                      }}
                    />
                  </td>
                  <td>
          ...
          <div className="ssc-form-btns">
            <button className="ssc-btn ssc-btn-primary" onClick={handleUpdate}>
              Update
            </button>
          </div>
        </div>
  ...
  if (view === "new") {
    ...
    const lastClaim = claims
      .filter((c) => c.staffId === newStaff?.id)
      .sort((a, b) => b.id - a.id)[0];
    const addRow = () =>
      setNewRows([...newRows, { from: "", upto: "", cl: "", lop: "" }]);
    const deleteRow = () => {
      if (newRows.length > 1) setNewRows(newRows.slice(0, -1));
    };
    const setRow = (i, k, v) => {
      const r = [...newRows];
      r[i] = { ...r[i], [k]: v };
    ...
    const handleAdd = () => {
      ...
      for (const r of newRows) {
        if (!r.from || !r.upto) return alert("Fill all date fields.");
      }
      const totalDays = newRows.reduce(
        (s, r) => s + daysBetween(r.from, r.upto),
        0,
      );
      const totalCL = newRows.reduce((s, r) => s + (parseFloat(r.cl) || 0), 0);
      const totalLOP = newRows.reduce(
        (s, r) => s + (parseFloat(r.lop) || 0),
        0,
      );
      const totalAmt = newRows.reduce(
        (s, r) =>
          s +
          calcNetSalary(
            r.from,
            r.upto,
            newStaff.salaryPerMonth,
            parseFloat(r.cl) || 0,
            parseFloat(r.lop) || 0,
          ),
        0,
      ...
      setClaims([
        ...claims,
        {
          ...
          rows: newRows.map((r) => ({
            from: r.from,
            upto: r.upto,
            cl: parseFloat(r.cl) || 0,
            lop: parseFloat(r.lop) || 0,
          })),
        },
      ]);
      setNextId(nextId + 1);
          ...
          <div className="ssc-find-row">
            <div className="ssc-field">
              ...
              <div className="ssc-ss-wrap">
                <div
                  className="ssc-ss-trigger"
                  onClick={() => setStaffDropOpen(!staffDropOpen)}
                >
                  <span className={newStaff ? "" : "ssc-placeholder"}>
                    {newStaff
                      ? `${newStaff.salutation} ${newStaff.name}`
                      : "--Select--"}
                  </span>
                  <span>▾</span>
                </div>
                {staffDropOpen && (
                  <div className="ssc-ss-dropdown">
                    ...
                    />
                    <div
                      className={`ssc-ss-option ${!newStaff ? "ssc-ss-selected" : ""}`}
                      onClick={() => {
                        setNewStaff(null);
                    ...
                    {filtered.map((s) => (
                      <div
                        key={s.id}
                        className={`ssc-ss-option ${newStaff?.id === s.id ? "ssc-ss-selected" : ""}`}
                        onClick={() => {
                          setNewStaff(s);
                          setStaffDropOpen(false);
                          setNewStaffSearch("");
          ...
          {newStaff && (
            <>
              <div className="ssc-info-block" style={{ margin: "12px 0" }}>
                <div>
                  <strong>Appointment:</strong> {newStaff.appointmentFrom} to{" "}
                  {newStaff.appointmentTo}
                </div>
                <div>
                  <strong>Salary claimed upto:</strong>{" "}
                  {lastClaim?.salaryTo || "Not claimed yet"}
                </div>
                <div>
                  <strong>Total CL availed:</strong>{" "}
                  {claims
                    .filter((c) => c.staffId === newStaff.id)
                    .reduce((s, c) => s + c.clDays, 0)}
                </div>
              </div>
              <table className="ssc-rows-table ssc-rows-table-new">
                <thead>
                  <tr>
                    <th style={{ color: "#38bdf8" }}>Claim from</th>
                    <th style={{ color: "#38bdf8" }}>Upto</th>
                    <th style={{ color: "#38bdf8" }}>CL</th>
                    <th style={{ color: "#38bdf8" }}>LOP</th>
                  </tr>
                </thead>
                <tbody>
                  {newRows.map((row, i) => (
                    <tr key={i}>
                      ...
                      <td>
                        <input
                          ...
                          value={
                            row.upto
                              ? row.upto.split("-").reverse().join("-")
                              : ""
                          }
                          onChange={(e) => {
                            const p = e.target.value.split("-");
                            setRow(i, "upto", p.reverse().join("-"));
                          }}
                        />
                      </td>
                      <td>
                      ...
                      <td>
                        <input
                          ...
                          className="ssc-input"
                          value={row.lop}
                          onChange={(e) => setRow(i, "lop", e.target.value)}
                        />
                      </td>
                    </tr>
                  ))}
              ...
              <div className="ssc-row-actions">
                <button className="ssc-btn ssc-btn-outline" onClick={addRow}>
                  + Add Row
                </button>
                <button className="ssc-btn ssc-btn-outline" onClick={deleteRow}>
                  − Delete Row
                </button>
              </div>
              <div className="ssc-form-btns">
                <button className="ssc-btn ssc-btn-primary" onClick={handleAdd}>
                  Add
                </button>
              </div>
            </>
...
/* ─── ConsumablesPage (fixed) ───
   ...
   1. Budget Allotment / Amount Incurred were always ₹0 because BalanceBar
      fell back to project.budgetAllotment / project.amountIncurredSoFar,
      neither of which exists on the real fetched project object. Now
      fetches the real numbers from
      GET /api/claim-projects/recurring/consumables/:projectId (a route
      that already existed on the backend but was never called from here).
   2. handleSubmit only updated local React state — it never POSTed to
      /api/claim-submit/consumables, so a submitted claim never actually
      persisted, meaning "claimed so far" could never grow to reflect it.
      Now submits for real, matching NonRecurringEquipmentPage's pattern,
      and passes the saved DB row up to onSubmit. */
function ConsumablesPage({ project, onSubmit, onBack }) {
  ...
  const [budgetInfo, setBudgetInfo] = useState({ budget: 0, claimedSoFar: 0 });
  const [submitting, setSubmitting] = useState(false);
  const set = (k, v) => setData((p) => ({ ...p, [k]: v }));

  useEffect(() => {
    fetch(
      `http://localhost:5000/api/claim-projects/recurring/consumables/${project.id}`,
    )
      .then((r) => r.json())
      .then((d) => {
        setBudgetInfo({ budget: d.budget, claimedSoFar: d.claimedSoFar });
      })
      .catch(console.error);
  }, [project.id]);

  const getCombinedHTML = () =>
    buildCombinedReport("consumables", data, project, null, null);
  ...
  const handleDownload = async () => {
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      pdf.addImage(
        ...
        0,
        pw,
        Math.min((canvas.height * pw) / canvas.width, 297),
      );
    }
    document.body.removeChild(iframe);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        "http://localhost:5000/api/claim-submit/consumables",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            project_id: project.id,
            proceeding_no: data.proceedingNo,
            division_label: data.divisionLabel,
            item_description: data.itemDescription,
            vendor_name: data.vendorName,
            vendor_address: data.vendorAddress,
            vendor_city: data.vendorCity,
            sanctioning_authority: data.sanctioningAuthority,
            mh_no: data.mhNo,
            financial_year: data.financialYear,
            sanction_page_no: data.sanctionPageNo,
            sanction_sl_no: data.sanctionSlNo,
            amount: Number(data.amount),
            report_html: getCombinedHTML(),
          }),
        },
      );
      if (!res.ok) throw new Error("Failed to submit claim");
      const saved = await res.json();
      onSubmit(saved, "Consumables & Accessories");
    } catch (err) {
      console.error(err);
  ...
  return (
    <>
      ...
      >
          ...
          <div className="cons-section-body">
            <div className="cons-grid-2" style={{ gap: 16, marginBottom: 16 }}>
              ...
              <div className="cons-field">
                ...
                <input
                  type="text"
                  className="slip-input"
                  placeholder="e.g. chemicals, lab consumables..."
            ...
            {data.amount && Number(data.amount) > 0 && (
              <div className="cons-field" style={{ marginBottom: 16 }}>
                <label>
                  Amount in Words{" "}
                  <span className="src-badge src-auto">Auto</span>
                </label>
                <div className="cons-static">
            ...
            <BalanceBar
              ...
              claimedSoFar={budgetInfo.claimedSoFar}
              amount={data.amount}
              budgetLabel="Budget Allotment (Consumables)"
              incurredLabel="Amount Incurred (incl. this)"
            />
          </div>
        </div>
        ...
        <div className="cons-section">
          ...
          <div className="cons-section-body">
            <div className="cons-grid-2" style={{ gap: 16 }}>
              <div className="cons-field">
                <label>
                  Vendor Name (M/s.) <span className="req">*</span>{" "}
                  <span className="src-badge src-enter">Enter</span>
                </label>
                <input
                  type="text"
                  className="slip-input"
                  placeholder="e.g. Sri Hari Scientific"
              ...
              <div className="cons-field">
                ...
                <input
                  type="text"
                  className="slip-input"
                  placeholder="e.g. Chennai – 600100"
                  value={data.vendorCity}
                  onChange={(e) => set("vendorCity", e.target.value)}
                />
              </div>
              <div className="cons-field" style={{ gridColumn: "1 / -1" }}>
                ...
                <textarea
                  className="slip-input"
                  rows={2}
                  placeholder="No.1 Sample Street, Chennai - 600100"
        ...
        <div className="cons-action-row">
          <div className="cons-action-left">
            <button className="cons-preview-btn" onClick={handlePreview}>
              👁 Preview Full Report
            </button>
            <button className="cons-download-btn" onClick={handleDownload}>
              ⬇ Download PDF
            </button>
          </div>
          <button
            className="cons-submit-btn"
      ...
      {preview && previewHTML && (
        <div
          className="cons-preview-overlay"
          onClick={(e) => {
        ...
        >
          <div className="cons-preview-box">
            <div className="cons-preview-head">
              <span>
                📄 Consumables Claim — {project.id} (Cover + Proceedings)
              </span>
              <div>
                <button className="btn-dl" onClick={handleDownload}>
                  ⬇ Download
                </button>
                <button className="btn-cl" onClick={() => setPreview(false)}>
                  ✕ Close
                </button>
              </div>
            </div>
            <iframe
              className="cons-preview-iframe"
              srcDoc={previewHTML}
              title="Consumables Full Report"
            />
          </div>
        </div>
      )}
...
/* ─── TravelPage ─── */
/* ─── TravelPage (fixed) ───
   Same two fixes as ConsumablesPage:
   1. Budget Allotment / Amount Incurred were always ₹0 (fell back to
      project.budgetAllotment / project.amountIncurredSoFar, neither of
      which exists on the real fetched project object). Now fetches real
      numbers from GET /api/claim-projects/recurring/travel/:projectId.
   2. handleSubmit only updated local React state — now actually POSTs to
      /api/claim-submit/travel so the claim persists and "claimed so far"
      reflects it on future loads. */
function TravelPage({ project, onSubmit, onBack }) {
  const [data, setData] = useState({
    ...
    fromPlace: "Chennai",
    toPlace: "",
    purpose: "Data collection field work",
    payeeName: "",
    headOfAccount: "Travel",
    divisionLabel: "",
    startKm: "",
    endKm: "",
    sanctioningAuthority: "Director of Technical Education (DoTE), Chennai",
  ...
  useEffect(() => {
    fetch(`http://localhost:5000/api/claim-projects/recurring/travel/${project.id}`)
      .then((r) => r.json())
      .then((d) => {
        setBudgetInfo({ budget: d.budget, claimedSoFar: d.claimedSoFar });
      })
  ...
  const validate = () => {
    if (
      ...
      alert(
        "Please fill Proceeding No., Amount, Destination, Travel Date and Payee Name.",
      );
      return false;
    }
    return true;
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("http://localhost:5000/api/claim-submit/travel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          project_id: project.id,
          ...
          start_km: data.startKm || null,
          end_km: data.endKm || null,
          head_of_account: data.headOfAccount,
          financial_year: data.financialYear,
          sanction_page_no: data.sanctionPageNo,
          sanction_sl_no: data.sanctionSlNo,
          amount: Number(data.amount),
      ...
      >
        ...
        <div className="cons-section">
          ...
          <div className="cons-section-body">
            <div className="cons-grid-3" style={{ gap: 16, marginBottom: 16 }}>
              <div className="cons-field">
                <label>
                  From Place <span className="src-badge src-enter">Enter</span>
                </label>
                <input
                  type="text"
                  className="slip-input"
                  ...
                  onChange={(e) => set("fromPlace", e.target.value)}
                />
              </div>
              <div className="cons-field">
                <label>
                  To Place (Destination) <span className="req">*</span>{" "}
                  <span className="src-badge src-enter">Enter</span>
                </label>
                <input
                  type="text"
                  className="slip-input"
                  placeholder="e.g. Tanjore"
                  value={data.toPlace}
                  onChange={(e) => set("toPlace", e.target.value)}
                />
              </div>
              <div className="cons-field">
                <label>
                ...
                <select
                  ...
                  onChange={(e) => set("modeOfTravel", e.target.value)}
                >
            ...
            <div className="cons-grid-3" style={{ gap: 16, marginBottom: 16 }}>
              <div className="cons-field">
                <label>
                  Vehicle No. <span className="src-badge src-enter">Enter</span>
                </label>
                <input
                  type="text"
                  className="slip-input"
                  placeholder="e.g. TN 14 D 1651"
              ...
              <div className="cons-field">
                <label>
                  Duration <span className="src-badge src-enter">Enter</span>
                </label>
                <input
                  type="text"
                  className="slip-input"
            ...
            <div className="cons-grid-2" style={{ gap: 16, marginBottom: 16 }}>
              <div className="cons-field" style={{ gridColumn: "1 / -1" }}>
                ...
                <input
                  type="text"
                  className="slip-input"
                  placeholder="e.g. Data collection field work"
                  value={data.purpose}
              ...
              <div className="cons-field">
                <label>
                  Payee Name (NEFT in favour of) <span className="req">*</span>{" "}
                  <span className="src-badge src-enter">Enter</span>
                </label>
                <input
                  type="text"
                  className="slip-input"
                  placeholder="e.g. P. Uma Maheswari"
            ...
            <BalanceBar
              ...
              claimedSoFar={budgetInfo.claimedSoFar}
              amount={data.amount}
              budgetLabel="Budget Allotment (Travel)"
              incurredLabel="Amount Incurred (incl. this)"
            />
          </div>
        </div>
        ...
        <div className="cons-section">
          <div className="cons-section-head">
            ...
            <h3>Odometer Readings (for own vehicle)</h3>
          </div>
          <div className="cons-section-body">
            <div className="cons-grid-2" style={{ gap: 16 }}>
              <div className="cons-field">
            ...
            {data.startKm &&
              data.endKm &&
              Number(data.endKm) > Number(data.startKm) && (
                <div className="cons-field" style={{ marginTop: 16 }}>
                  ...
                  <div className="cons-static">
                    Total{" "}
                    <strong>
                      {Number(data.endKm) - Number(data.startKm)} Km
                    </strong>
                  </div>
                </div>
              )}
...
function ContingencyPage({ project, onSubmit, onBack }) {
  const [data, setData] = useState({
    proceedingNo: "",
    amount: "",
    itemDescription: "contingency items",
    divisionLabel: "",
    sanctioningAuthority: "Director of Technical Education (DoTE), Chennai",
    mhNo: "16.1.17",
    financialYear: "2025 – 26",
  ...
  useEffect(() => {
    fetch(
      `http://localhost:5000/api/claim-projects/recurring/contingency/${project.id}`,
    )
      .then((r) => r.json())
      .then((d) => {
        setBudgetInfo({ budget: d.budget, claimedSoFar: d.claimedSoFar });
  ...
  const handlePreview = () => {
    if (!data.proceedingNo || !data.amount) {
      alert("Please fill Proceeding No. and Amount.");
      return;
    }
    setPreviewHTML(getCombinedHTML());
    setPreview(true);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        "http://localhost:5000/api/claim-submit/contingency",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        <div className="cons-section">
          ...
          <div className="cons-section-body">
            <div className="cons-grid-2" style={{ gap: 16, marginBottom: 16 }}>
              ...
              <div className="cons-field">
                ...
                <input
                  type="text"
                  className="slip-input"
                  placeholder="e.g. printing, postage, stationery..."
                  value={data.itemDescription}
                  onChange={(e) => set("itemDescription", e.target.value)}
                />
              </div>
            ...
            <BalanceBar
              ...
              claimedSoFar={budgetInfo.claimedSoFar}
              amount={data.amount}
              budgetLabel="Budget Allotment (Contingency)"
              incurredLabel="Amount Incurred (incl. this)"
            />
          </div>
        </div>
      ...
      {preview && previewHTML && (
        ...
        >
          <div className="cons-preview-box">
            <div className="cons-preview-head">
              <span>
                📄 Contingency Claim — {project.id} (Cover + Proceedings)
              </span>
              <div>
                <button className="btn-dl" onClick={handleDownload}>
                  ⬇ Download
...
function OverheadPage({ project, onSubmit, onBack }) {
  ...
  useEffect(() => {
    fetch(`http://localhost:5000/api/claim-projects/overhead/${project.id}`)
      .then((r) => r.json())
      .then((d) => {
        const mapped = (d.items || []).map((it) => ({
          ...
          percent: it.percent,
          amount: it.budget,
          claimedSoFar: it.claimedSoFar,
          balance: it.balance,
        }));
        setItems(mapped);
        setSelected(mapped.map((i) => i.key));
      })
      .catch(console.error)
  ...
  const selectedTotal = selectedItems.reduce((s, i) => s + Number(i.amount || 0), 0);

  const getCombinedHTML = () =>
    buildCombinedReport(
      "overhead",
      { selectedOverheads: selected, overheadItems: items },
      project,
      null,
      null,
    );
  ...
  const handlePreview = () => {
    if (selected.length === 0) {
      alert("Please select at least one overhead head.");
      return;
    }
    setPreviewHTML(getCombinedHTML());
    setPreview(true);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("http://localhost:5000/api/claim-submit/overhead", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          project_id: project.id,
    ...
    <>
      ...
      >
        ...
        <div className="cons-header">
          ...
          <p>
            Select which overhead head(s) to process. Each selected head
            generates its own Cover Page + Proceedings (2 pages each).
          </p>
        </div>
        <ProjectDetailSection project={project} />
        ...
        <div className="cons-section">
          <div className="cons-section-head">
            ...
            >
              Fetched from overhead sanction — choose which to claim
            </span>
          </div>
          <div className="cons-section-body">
            {loading ? (
              <div style={{ fontSize: 13, color: "rgba(20,30,70,0.5)" }}>
                Loading overhead details…
              </div>
            ) : items.length === 0 ? (
              <div style={{ fontSize: 13, color: "#dc2626" }}>
                No overhead entries found for this project's latest
                installment.
              </div>
            ) : (
              <>
                  ...
                  {items.map((item, idx) => (
                    <label
                      ...
                      style={{
                        ...
                        alignItems: "center",
                        gap: 10,
                        background: selected.includes(item.key)
                          ? "rgba(124,58,237,0.07)"
                          : "rgba(0,100,220,0.03)",
                        border: selected.includes(item.key)
                          ? "1px solid rgba(124,58,237,0.3)"
                          : "1px solid rgba(0,100,220,0.1)",
                        borderRadius: 12,
                        padding: "12px 14px",
                        cursor: "pointer",
                        transition: "0.2s",
                      ...
                      <div>
                        ...
                        <div
                          style={{
                            ...
                            fontSize: 11,
                            color:
                              item.balance >= 0 ? "#64748b" : "#dc2626",
                            marginTop: 2,
                          }}
                        >
                          Balance: Rs. {fmtAmt(item.balance)}/-
                        </div>
                      </div>
                    </label>
                  ))}
                ...
                <div className="cons-field" style={{ marginTop: 18 }}>
                  ...
                  <div className="cons-static">
                    <strong>Rs. {fmtAmt(selectedTotal)}/-</strong>
                    {selectedTotal > 0 && (
                      <> (Rupees {toIndianWords(selectedTotal)})</>
                    )}
                  </div>
                </div>
                <div className="cons-field" style={{ marginTop: 10 }}>
                  ...
                  <div className="cons-static">
                    {selected.length} head(s) × 2 pages ={" "}
                    <strong>{selected.length * 2} pages</strong>
                  </div>
                </div>
              </>
            )}
      ...
      {preview && previewHTML && (
        ...
        >
          <div className="cons-preview-box">
            <div className="cons-preview-head">
              <span>
                📄 Overhead Claim — {project.id} ({selected.length * 2} pages)
              </span>
              <div>
                <button className="btn-dl" onClick={handleDownload}>
                  ⬇ Download
...
function OtherExpensesPage({ project, onSubmit, onBack }) {
  ...
  const set = (k, v) => setData((p) => ({ ...p, [k]: v }));
  const getCombinedHTML = () =>
    buildCombinedReport("otherExpenses", data, project, null, null);
  const handlePreview = () => {
    if (!data.proceedingNo || !data.amount || !data.purchaseOf) {
      alert("Please fill Proceeding No., Amount, and Purchase of (what).");
      return;
    }
    setPreviewHTML(getCombinedHTML());
    setPreview(true);
  ...
  const handleSubmit = () => {
    ...
    onSubmit({ ...data, _reportHTML: getCombinedHTML() }, "Other Expenses");
  };
      ...
      >
        ...
        <div className="cons-section">
          ...
          <div className="cons-section-body">
            <div className="cons-grid-2" style={{ gap: 16, marginBottom: 16 }}>
              <div className="cons-field">
                <label>
                  Purchase of (What?) <span className="req">*</span>{" "}
                  <span className="src-badge src-enter">Enter</span>
                </label>
                <input
                  type="text"
                  className="slip-input"
                  placeholder="e.g. Publication charges, Patent filing fees..."
                  value={data.purchaseOf}
                  onChange={(e) => set("purchaseOf", e.target.value)}
                />
              </div>
              <div className="cons-field">
                <label>
        ...
        <div className="cons-section">
          <div className="cons-section-head">
            ...
            <h3>Vendor / Payee Details</h3>
          </div>
          <div className="cons-section-body">
            <div className="cons-grid-2" style={{ gap: 16 }}>
              <div className="cons-field">
                <label>
                  Vendor / Payee Name (M/s.){" "}
                  <span className="src-badge src-enter">Enter</span>
                </label>
                <input
                  type="text"
                  className="slip-input"
                  placeholder="e.g. Springer, IEEE, Patent Office..."
                  value={data.vendorName}
                  onChange={(e) => set("vendorName", e.target.value)}
                />
              </div>
        ...
        <div className="cons-action-row">
          ...
          <button className="cons-submit-btn" onClick={handleSubmit}>
            ✓ Submit Claim →
          </button>
        </div>
      </div>
      {preview && previewHTML && (
        ...
        >
          <div className="cons-preview-box">
            <div className="cons-preview-head">
              <span>
                📄 Other Expenses Claim — {project.id} (Cover + Proceedings)
              </span>
              <div>
                <button className="btn-dl" onClick={handleDownload}>
                  ⬇ Download
            ...
            <iframe
              className="cons-preview-iframe"
              srcDoc={previewHTML}
              title="Other Expenses Full Report"
            />
          </div>
        </div>
      )}
...
function NonRecurringEquipmentPage({ project, onSubmit, onBack }) {
  ...
  const [equipmentBudget, setEquipmentBudget] = useState(0);
  const [submitting, setSubmitting] = useState(false);

  useEffect(() => {
    fetch(
      `http://localhost:5000/api/claim-projects/non-recurring/${project.id}`,
    )
      .then((r) => r.json())
      .then((d) => {
        setBudgetInfo({ budget: d.budget, claimedSoFar: d.claimedSoFar });
        setEquipmentOptions(d.equipmentOptions || []);
      })
      .catch(console.error);
  }, [project.id]);

  const set = (k, v) => setData((p) => ({ ...p, [k]: v }));
  const handleEquipmentSelect = (equipmentIdStr) => {
    const head = equipmentOptions.find((e) => String(e.id) === equipmentIdStr);
    setData((p) => ({
      ...p,
      nonRecurringHeadId: head ? head.id : null,
      equipmentName: head ? head.equipment : "",
      amount: "",
    }));
    setEquipmentBudget(head ? Number(head.amount) : 0);
  };
  const getCombinedHTML = () =>
    buildCombinedReport(
      "nonRecurring",
  ...
  const handlePreview = () => {
    if (!data.proceedingNo || !data.amount || !data.equipmentName) {
      alert("Please fill Proceeding No., Amount, and Equipment Name.");
      return;
    }
    setPreviewHTML(getCombinedHTML());
    setPreview(true);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        "http://localhost:5000/api/claim-submit/non-recurring",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            project_id: project.id,
            non_recurring_head_id: data.nonRecurringHeadId || null,
      ...
      if (!res.ok) throw new Error("Failed to submit claim");
      const saved = await res.json();
      onSubmit(saved, data.equipmentName);
    } catch (err) {
      console.error(err);
      alert("Could not submit claim. Please try again.");
    } finally {
      ...
      >
        ...
        <div className="cons-section">
          ...
          <div className="cons-section-body">
            <div className="cons-grid-2" style={{ gap: 16 }}>
              <div className="cons-field">
                <label>
                  HOD Name <span className="src-badge src-enter">Enter</span>
                </label>
                <input
                  type="text"
                  className="slip-input"
                  placeholder="e.g. Dr. S. Senthil Kumaran"
                  value={data.hodName}
              ...
              <div className="cons-field">
                ...
                <input
                  type="text"
                  className="slip-input"
                  placeholder="e.g. 02 / RAC /CMRG/ 2025-26"
                  value={data.proceedingNo}
                  onChange={(e) => set("proceedingNo", e.target.value)}
                />
              </div>
        ...
        <div className="cons-section">
          <div className="cons-section-head">
            ...
            <h3>Equipment & Expenditure Details</h3>
          </div>
          <div className="cons-section-body">
            <div className="cons-grid-2" style={{ gap: 16, marginBottom: 16 }}>
              <div className="cons-field" style={{ gridColumn: "1 / -1" }}>
                <label>
                  Equipment Name <span className="req">*</span>{" "}
                  <span className="src-badge src-fetch">Fetched</span>
                </label>
                <select
                  className="slip-input"
                  value={
                    data.nonRecurringHeadId != null
                      ? String(data.nonRecurringHeadId)
                      : ""
                  }
                ...
                >
                  ...
                  {equipmentOptions.map((eq) => (
                    <option key={eq.id} value={eq.id}>
                      {eq.equipment} (Rs.{" "}
                      {Number(eq.amount).toLocaleString("en-IN")}/-)
                    </option>
                  ))}
                </select>
                {equipmentOptions.length === 0 && (
                  <div style={{ fontSize: 12, color: "#dc2626", marginTop: 4 }}>
                    No equipment heads found for this project's latest
                    installment.
                  </div>
                )}
              </div>
              ...
              <div className="cons-field">
                <label>
                  Vendor / Supplier Name (M/s.){" "}
                  <span className="src-badge src-enter">Enter</span>
                </label>
                <input
                  type="text"
                  className="slip-input"
                  placeholder="e.g. Acurel weighing systems Pvt. Ltd."
                  value={data.vendorName}
                  onChange={(e) => set("vendorName", e.target.value)}
                />
              </div>
            ...
            <BalanceBar
              ...
              claimedSoFar={0}
              amount={data.amount}
              budgetLabel="Budget Allocated"
              incurredLabel="Amount Incurred"
            />
          </div>
        </div>
      ...
      {preview && previewHTML && (
        ...
        >
          <div className="cons-preview-box">
            <div className="cons-preview-head">
              <span>
                📄 Equipment Claim — {project.id} (Cover + Proceedings)
              </span>
              <div>
                <button className="btn-dl" onClick={handleDownload}>
                  ⬇ Download
...
export default function ZBASlipPage() {
  ...
  useEffect(() => {
    ...
    if (!userId) {
      console.error("No user found in sessionStorage — is the user logged in?");
      return;
    }
    fetch(`http://localhost:5000/api/claim-projects/my-projects/${userId}`)
      .then((r) => r.json())
      .then((rows) => {
        if (!Array.isArray(rows)) {
          console.error("Unexpected response from my-projects:", rows);
          return [];
        }
        return rows.map((r) => ({
          id: r.id,
          ...
          csrcProcNo: r.csrc_proc_no,
          csrcProcDate: r.csrc_proc_date,
          pi: r.pi_name,
          piDesignation: r.pi_designation,
          department: r.department,
          departmentFull: `Department of ${r.department}`,
          campus: r.campus,
  ...
  // ...rest of the existing useState declarations stay unchanged
  ...
  const generateReportPDF = async (claim, mode = "preview") => {
    ...
    el.style.cssText =
      "position:fixed;left:-9999px;top:0;width:900px;padding:30px;background:#fff;";
  ...
  const renderHeadType = () => (
    <div className="slip-card" style={{ animation: "slideIn 0.3s ease" }}>
      <div className="claim-breadcrumb">
        <span className="bc-link" onClick={() => setScreen("projects")}>
          Projects
        </span>
      ...
      <div className="head-type-grid">
        <div
          className="head-type-card non-recurring"
          onClick={() => setScreen("nonRecurring")}
        >
          <div className="hc-icon">🔧</div>
          <div className="hc-title">Non-Recurring</div>
          <div className="hc-sub">
            Equipment purchases, one-time capital expenditure items
          </div>
          <div className="hc-arr">→</div>
        </div>
        <div
          className="head-type-card recurring"
          onClick={() => setScreen("recurring")}
        >
          <div className="hc-icon">🔄</div>
          <div className="hc-title">Recurring</div>
          <div className="hc-sub">
            Manpower, consumables, travel, contingency & other expenses
          </div>
          <div className="hc-arr">→</div>
        </div>
      </div>
      <div style={{ marginTop: 16, display: "flex", gap: 10 }}>
        <button className="back-btn" onClick={() => setScreen("projects")}>
          ← Back to Projects
        </button>
        <button
          className="settled-btn"
          style={{ marginLeft: 0 }}
          onClick={() => setScreen("underReview")}
        >
  ...
  const RECURRING_HEADS = [
    {
      ...
      label: "Manpower",
      icon: "👥",
      sub: "Salary claims for JRF/SRF/RA",
    },
    {
      key: "consumables",
      label: "Consumables & Accessories",
      icon: "🧪",
      sub: "Lab materials, chemicals, supplies",
    },
    {
      key: "travel",
      label: "Travel",
      icon: "✈️",
      sub: "Conference, field work, official visits",
    },
    {
      key: "contingency",
      label: "Contingency",
      icon: "📦",
      sub: "Postage, printing, stationery, misc",
    },
    {
      key: "overhead",
      label: "Overhead",
      icon: "🏛️",
      sub: "Auto-computed institutional overhead (15%)",
    },
    {
      key: "otherExpenses",
      label: "Other Expenses",
      icon: "💰",
      sub: "Publications, patents, other misc.",
    },
  ];
  ...
  const renderRecurring = () => (
    <div className="slip-card" style={{ animation: "slideIn 0.3s ease" }}>
      <div className="claim-breadcrumb">
        ...
        <span className="bc-sep">›</span>
        <span className="bc-link" onClick={() => setScreen("headType")}>
          Update Claim
        </span>
        <span className="bc-sep">›</span>
      ...
      <div className="rec-head-grid">
        {RECURRING_HEADS.map((h) => (
          <div
            key={h.key}
            className="rec-head-card"
            onClick={() => {
              setRecurringHead(h.key);
              setScreen("recurringHead");
            }}
          >
            <div className="rhc-icon">{h.icon}</div>
            <div className="rhc-title">{h.label}</div>
            <div className="hc-sub" style={{ fontSize: 11 }}>
              {h.sub}
            </div>
          </div>
        ))}
  ...
  const renderUnderReview = () => {
    const displayClaims =
      reviewTab === "review" ? reviewClaims : approvedClaims;
    return (
      <div className="slip-card" style={{ animation: "slideIn 0.3s ease" }}>
        <div className="claim-breadcrumb">
          <span className="bc-link" onClick={() => setScreen("projects")}>
        ...
        <div className="review-tabs">
          <button
            className={`review-tab ${reviewTab === "review" ? "active-under" : ""}`}
            onClick={() => setReviewTab("review")}
          >
            ⏳ Under Review{" "}
            <span className="review-tab-count">{reviewClaims.length}</span>
          </button>
          <button
            className={`review-tab ${reviewTab === "approved" ? "active-approved" : ""}`}
            onClick={() => setReviewTab("approved")}
          >
            ✅ Approved{" "}
            <span className="review-tab-count">{approvedClaims.length}</span>
          </button>
        </div>
        {displayClaims.length === 0 ? (
          ...
          <div
            style={{
              overflowX: "auto",
              borderRadius: 14,
              border: "1px solid rgba(0,100,220,0.07)",
            }}
          >
            <table className="review-table">
              ...
              <tbody>
                {displayClaims.map((c, i) => (
                  <tr key={c.id}>
                    <td
                      style={{ color: "rgba(20,30,70,0.4)", fontWeight: 700 }}
                    >
                      {i + 1}
                    </td>
                    <td>{c.date}</td>
                    ...
                    <td>
                      <span className="head-badge-purple">{c.head}</span>
                    </td>
                    ...
                    <td>
                      <div className="review-action-group">
                        ...
                        <button
                          ...
                          onClick={() => generateReportPDF(c, "download")}
                        >
                          ⬇ Download
                        </button>
                        {c.status === "review" && (
                          <button
                            ...
                            onClick={() =>
                              approveClaim(selectedProject.id, c.id)
                            }
                          >
                            ✓ Approve
                          </button>
  ...
  const renderSettledModal = () => {
    ...
    const activeClaims = projectClaims(settledTab).filter(
      (c) => c.status === "approved",
    );
      ...
      >
        <div className="settled-modal">
          <div className="settled-modal-head">
            <h2>
              📋 Settled Bills <span>{activeProj?.title}</span>
            </h2>
          ...
          <div className="settled-modal-body">
            <div className="settled-tabs">
              {projects.map((p) => (
                <button
                  key={p.id}
                  className={`settled-tab ${settledTab === p.id ? "active" : ""}`}
                  onClick={() => setSettledTab(p.id)}
                >
                  {p.projectNo} — {p.title}
                  {projectClaims(p.id).filter((c) => c.status === "approved")
                    .length > 0 && (
                    <span
                      style={{
                        marginLeft: 6,
            ...
            <div className="settled-summary">
              <div className="settled-summary-card">
                <p>Total Approved</p>
                <h4>{activeClaims.length}</h4>
              </div>
              <div className="settled-summary-card">
                <p>Total Claimed</p>
                <h4 className="green">{fmt(total)}</h4>
              </div>
              <div className="settled-summary-card">
                <p>Balance Remaining</p>
                <h4 className="yellow">
                  {fmt((activeProj?.sanctionedAmount || 0) - total)}
                </h4>
              </div>
            </div>
            {activeClaims.length === 0 ? (
              <div className="empty-bills">
                <div className="empty-icon">📭</div>No settled bills found.
              </div>
            ) : (
