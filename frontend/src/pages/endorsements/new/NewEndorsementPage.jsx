...
export default function NewEndorsementPage() {
  ...
  const [coPIs,   setCoPIs]   = useState([{ ...initialCoPi }]);
  const [extInvs, setExtInvs] = useState([{ ...initialExtInv }]);
  const [files,   setFiles]   = useState({
    proposal: null, writeup: null, budget: null, endorsementFile: null, overhead: null,
  });
  ...
  const updateCoPi = (i, k, v) =>
    setCoPIs(prev => {
      const next = [...prev];
      next[i] = { ...next[i], [k]: v };
      if (k === "campus")     { next[i].department = ""; next[i].faculty = ""; }
      if (k === "department") { next[i].faculty = ""; }
      return next;
    });
  const addCoPiRow  = () => setCoPIs(p => [...p, { ...initialCoPi }]);
  const delCoPiRow  = () => setCoPIs(p => (p.length > 1 ? p.slice(0, -1) : p));

  const updateExtInv = (i, k, v) =>
    setExtInvs(prev => { const n = [...prev]; n[i] = { ...n[i], [k]: v }; return n; });
  const addExtRow  = () => setExtInvs(p => [...p, { ...initialExtInv }]);
  const delExtRow  = () => setExtInvs(p => (p.length > 1 ? p.slice(0, -1) : p));
    ...
    <div className={`ep-page ${mounted ? "ep-loaded" : ""}`}>
      <div className="ep-container">
        ...
        <div className="ep-top-nav">
          <button className="ep-btn-back" onClick={goBack}>
        ...
        <div className="ep-header-banner">
          <div className="ep-header-content">
            ...
            <p className="ep-header-sub">Centre for Sponsored Research and Consultancy — Anna University</p>
          </div>
        ...
        <div className="ep-card ep-anim-1">
          ...
          <div className="ep-grid-2">
            <div className="ep-input-group">
              ...
              <select className="ep-select" value={form.fundingAgency} onChange={e => setField("fundingAgency", e.target.value)}>
            ...
            <div className="ep-input-group">
              ...
              <select className="ep-select" value={form.projectScheme} onChange={e => setField("projectScheme", e.target.value)}>
          ...
          <div className="ep-grid-2 ep-mt24">
            <div className="ep-radio-card">
              ...
              <div className="ep-radio-group">
                {["Central Govt", "State Govt", "Private", "Individual"].map(label => (
                  <label key={label} className="ep-custom-radio-label">
                    <input type="radio" name="fundingType" value={label} checked={form.fundingType === label} onChange={e => setField("fundingType", e.target.value)} />
            ...
            <div className="ep-radio-card">
              ...
              <div className="ep-radio-group">
                {["Academic", "Collaborative", "International"].map(label => (
                  <label key={label} className="ep-custom-radio-label">
                    <input type="radio" name="projectType" value={label} checked={form.projectType === label} onChange={e => setField("projectType", e.target.value)} />
                    <span className="ep-radio-circle"></span>
                    {label}
                  </label>
                ))}
        ...
        <div className="ep-card ep-anim-2">
          ...
          <div className="ep-input-group">
            ...
            <textarea className="ep-textarea" placeholder="Enter the complete title of the proposal..." value={form.title} onChange={e => setField("title", e.target.value)} />
          </div>
          <div className="ep-input-group ep-mt16">
            <label className="ep-label">Reference Number <span className="ep-hint">(optional)</span></label>
            <input className="ep-input ep-input-sm" placeholder="e.g. 2526ET0937/CSRC-2" value={form.refNo} onChange={e => setField("refNo", e.target.value)} />
          </div>
        </div>
        ...
        <div className="ep-card ep-anim-3">
          ...
          <div className="ep-grid-3">
            <div className="ep-input-group">
              <label className="ep-label">Non-Recurring (₹) <span className="ep-hint">Equipment</span></label>
              <input className="ep-input" type="number" placeholder="0" value={form.nonRecurring} onChange={e => setField("nonRecurring", e.target.value)} />
            </div>
            <div className="ep-input-group">
              <label className="ep-label">Recurring (₹) <span className="ep-hint">Salary, Consumables</span></label>
              <input className="ep-input" type="number" placeholder="0" value={form.recurring} onChange={e => setField("recurring", e.target.value)} />
            </div>
            <div className="ep-input-group">
              <label className="ep-label">Overhead %</label>
              <input className="ep-input" type="number" value={form.overheadPct} onChange={e => setField("overheadPct", e.target.value)} />
            </div>
          </div>
          ...
          <div className="ep-gst-toggle-wrap ep-mt24">
            ...
            <div className="ep-toggle-buttons">
              <button className={`ep-toggle-btn ${form.gst === "yes" ? "active" : ""}`} onClick={() => setField("gst", "yes")}>Yes, Include GST</button>
              <button className={`ep-toggle-btn ${form.gst === "no" ? "active danger" : ""}`} onClick={() => setField("gst", "no")}>No GST</button>
            </div>
          </div>
          ...
          <div className="ep-calc-box">
            <div className="ep-calc-header">
              <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
              Dynamic Cost Summary
            </div>
            <div className="ep-calc-body">
              <div className="ep-calc-row"><span>Base Amount (Recurring + Non-Recurring)</span><span className="ep-calc-val">₹ {formatCurrency(baseAmount)}</span></div>
              <div className="ep-calc-row ep-calc-border"><span>Overhead ({form.overheadPct}%)</span><span className="ep-calc-val">₹ {formatCurrency(overheadAmt)}</span></div>
              <div className="ep-calc-row ep-calc-subtotal"><span>Subtotal</span><span className="ep-calc-val">₹ {formatCurrency(subTotal)}</span></div>
              
              <div className={`ep-calc-row ep-gst-row ${form.gst === 'yes' ? 'visible' : ''}`}>
                <span>GST (18% of Subtotal)</span>
                <span className="ep-calc-val">+ ₹ {formatCurrency(gstAmt)}</span>
              </div>
              
              <div className="ep-calc-row ep-calc-total">
                <span>Total Project Amount</span>
                <span className="ep-total-highlight">₹ {formatCurrency(grandTotal)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="ep-card ep-anim-4">
          ...
          <div className="ep-grid-3">
            <div className="ep-input-group">
              ...
              <input className="ep-input" type="date" value={form.dueDate} onChange={e => setField("dueDate", e.target.value)} />
            </div>
            <div className="ep-radio-card ep-radio-card-sm">
              ...
              <div className="ep-radio-group ep-radio-inline">
                <label className="ep-custom-radio-label"><input type="radio" name="piReg" value="yes" checked={form.isPIRegular === "yes"} onChange={() => setField("isPIRegular", "yes")} /><span className="ep-radio-circle"></span> Yes</label>
                <label className="ep-custom-radio-label"><input type="radio" name="piReg" value="no" checked={form.isPIRegular === "no"} onChange={() => setField("isPIRegular", "no")} /><span className="ep-radio-circle"></span> No</label>
              </div>
            </div>
            <div className="ep-radio-card ep-radio-card-sm">
              <label className="ep-label">Endorsement Required?</label>
              <div className="ep-radio-group ep-radio-inline">
                <label className="ep-custom-radio-label"><input type="radio" name="endReq" value="yes" checked={form.endorsementRequired === "yes"} onChange={() => setField("endorsementRequired", "yes")} /><span className="ep-radio-circle"></span> Yes</label>
                <label className="ep-custom-radio-label"><input type="radio" name="endReq" value="no" checked={form.endorsementRequired === "no"} onChange={() => setField("endorsementRequired", "no")} /><span className="ep-radio-circle"></span> No</label>
              </div>
            </div>
          </div>
          ...
          <div className="ep-grid-2 ep-mt24">
            <div className="ep-input-group">
              ...
              <select className="ep-select" value={form.endorsementFormat} onChange={e => setField("endorsementFormat", e.target.value)}>
                {ENDORSEMENT_FORMATS.map(f => <option key={f}>{f}</option>)}
              </select>
            </div>
            <div className="ep-input-group">
              <label className="ep-label">Years of Service Left Before Superannuation</label>
              <input className="ep-input" placeholder="e.g. 11" value={form.yearsService} onChange={e => setField("yearsService", e.target.value)} />
            </div>
          </div>

          <div className="ep-pi-bar">
            <div className="ep-pi-avatar">{form.piName.charAt(0)}</div>
            <div className="ep-pi-details">
              <div className="ep-pi-name">{form.piName} <span className="ep-pi-role">{form.piRole}</span></div>
              <div className="ep-pi-meta">{form.piDesignation}, Dept. of {form.piDept}, {form.piCampus}</div>
              <div className="ep-pi-stats">
                <span><i className="ep-icon-dot"></i> DoB: {form.piDob}</span>
                <span><i className="ep-icon-dot"></i> Service: {form.piService}</span>
                <span><i className="ep-icon-dot"></i> Superannuation: {form.piSuperannuation}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="ep-card ep-anim-5">
          <div className="ep-card-heading ep-flex-between">
            ...
            <div className="ep-row-btns-top">
              <button className="ep-btn-icon ep-btn-add" onClick={addCoPiRow} title="Add Row">+</button>
              <button className="ep-btn-icon ep-btn-del" onClick={delCoPiRow} disabled={coPIs.length <= 1} title="Remove Row">−</button>
            </div>
          </div>
          <div className="ep-table-wrap">
            <table className="ep-table">
              <thead><tr><th>Campus</th><th>Department</th><th>Faculty</th><th>DoB</th><th>Service</th><th>Superannuation</th><th>Role</th></tr></thead>
              <tbody>
                {coPIs.map((row, i) => {
                  const depts = row.campus ? Object.keys(CAMPUS_DATA[row.campus] || {}) : [];
                  const faculties = row.department ? (CAMPUS_DATA[row.campus]?.[row.department] || []) : [];
                  return (
                    <tr key={i} className="ep-table-row-anim">
                      <td><select className="ep-select ep-select-sm" value={row.campus} onChange={e => updateCoPi(i, "campus", e.target.value)}><option value="">Select</option>{Object.keys(CAMPUS_DATA).map(c => <option key={c}>{c}</option>)}</select></td>
                      <td><select className="ep-select ep-select-sm" value={row.department} onChange={e => updateCoPi(i, "department", e.target.value)} disabled={!row.campus}><option value="">Select</option>{depts.map(d => <option key={d}>{d}</option>)}</select></td>
                      <td><select className="ep-select ep-select-sm" value={row.faculty} onChange={e => updateCoPi(i, "faculty", e.target.value)} disabled={!row.department}><option value="">Select</option>{faculties.map(f => <option key={f}>{f}</option>)}</select></td>
                      <td><input className="ep-input ep-input-date" type="date" value={row.dob} onChange={e => updateCoPi(i, "dob", e.target.value)} /></td>
                      <td><input className="ep-input ep-input-date" type="date" value={row.service} onChange={e => updateCoPi(i, "service", e.target.value)} /></td>
                      <td><input className="ep-input ep-input-date" type="date" value={row.superannuation} onChange={e => updateCoPi(i, "superannuation", e.target.value)} /></td>
                      <td><select className="ep-select ep-select-sm" value={row.role} onChange={e => updateCoPi(i, "role", e.target.value)}>{CO_PI_ROLES.map(r => <option key={r}>{r}</option>)}</select></td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        <div className="ep-card ep-anim-6">
          <div className="ep-card-heading ep-flex-between">
            ...
            <div className="ep-row-btns-top">
              <button className="ep-btn-icon ep-btn-add" onClick={addExtRow} title="Add Row">+</button>
              <button className="ep-btn-icon ep-btn-del" onClick={delExtRow} disabled={extInvs.length <= 1} title="Remove Row">−</button>
            </div>
          </div>
          <div className="ep-table-wrap">
            <table className="ep-table">
              <thead><tr><th>Full Name</th><th>Designation</th><th>Institute</th></tr></thead>
              <tbody>
                {extInvs.map((row, i) => (
                  <tr key={i} className="ep-table-row-anim">
                    <td><input className="ep-input" placeholder="Dr. John Doe" value={row.name} onChange={e => updateExtInv(i, "name", e.target.value)} /></td>
                    <td><input className="ep-input" placeholder="Senior Scientist" value={row.designation} onChange={e => updateExtInv(i, "designation", e.target.value)} /></td>
                    <td><input className="ep-input" placeholder="IIT Madras" value={row.institute} onChange={e => updateExtInv(i, "institute", e.target.value)} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {[
              ...
              <div key={key} className="ep-upload-row">
                ...
                <div className="ep-upload-control">
                  <input type="file" className="ep-file-input" onChange={e => handleFile(key, e)} id={`file-${key}`} />
                  <label htmlFor={`file-${key}`} className={`ep-file-btn ${files[key] ? 'uploaded' : ''}`}>
                    {files[key] ? "Re-upload" : "Choose File"}
                  </label>
                  {files[key] && <span className="ep-file-badge">✓ {files[key].name}</span>}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="ep-generate-wrap ep-anim-8">
          <button className="ep-btn-generate ep-glow-effect" onClick={() => setShowReport(true)}>
            <span>Generate Official Report</span>
            <svg width="24" height="24" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
          </button>
        </div>
      ...
      {/* ══ REPORT MODAL ═════════════════════════════════════════════════════ */}
      {showReport && (
        <div className="ep-overlay" onClick={e => { if (e.target === e.currentTarget) setShowReport(false); }}>
          <div className="ep-modal">
            <div className="ep-modal-header">
              <h2 className="ep-modal-title">Endorsement Preview</h2>
              <button className="ep-btn-close" onClick={() => setShowReport(false)}>✕</button>
            </div>

            <div className="ep-format-bar">
              <div className="ep-format-tabs">
                {ENDORSEMENT_FORMATS.map(fmt => (
                  <button key={fmt} className={`ep-format-tab ${activeFormat === fmt ? "active" : ""}`} onClick={() => setActiveFormat(fmt)}>
                    {fmt}
                  </button>
                ))}
              </div>
